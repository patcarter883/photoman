  ...
  resize = (file, maxDimensions) => new Promise((resolve) => {
    ...
    image.onload = () => {
      ...
      EXIF.getData(image, () => {
        ...
        if (hasToBlobSupport) imageCanvas.toBlob(blob => resolve(blob), this.outputType || file.type)
        else resolve(this.toBlob(imageCanvas, this.outputType || file.type))
      })
    }
